...
func getUserByPhone(phone string) (*model.User, error) {
	...
	if err := db.Where(&model.User{Phone: phone}).First(&user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
...
func (s *AuthService) LoginWithRefresh(identity, password string, refreshTokenTTL time.Duration) (accessToken string, refreshToken string, err error) {
	// Get the user from the database
	var user *models.User
	if isValidEmail(identity) {
		user, err = s.userRepo.GetUserByEmail(identity)
	} else if isValidPhone(identity) {
		user, err = s.userRepo.GetUserByPhone(identity)
	} else {
